...
gst <- function(geneSets){
  ...
  df_1 <- sapply(geneSets, FUN = geneSetTest, statistics = fit$t[, 2], alternative = "mixed", type = "t") %>% as.data.frame() # mixed for t??? or does it have to be up/down/either?
...
create_camera_plots <- function(data.to.plot){
  ...
  plot <- ggplot() + 
    ...
    scale_color_gradientn("Up", colors = c("green4", "gray80"), limits = c(0, 0.05)) +
    new_scale_color() +
    geom_point(data.to.plot %>% filter(Direction == "Down"), mapping = aes(x = x.axis.down, y = factor(Gene_set), 
                                                                             size = Matches, color = PValue)) + 
    facet_grid(Database ~ ., space = "free_y", scales = "free") +
    scale_color_gradientn("Down", colors = c("red4", "gray80"), limits = c(0, 0.05)) +
    theme_bw() + 
